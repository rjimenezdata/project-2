...
```{r}
#| context: data
measles <- read_csv("measles_df_clean.csv")
measles <- measles |>
  rename(`Nominal GDP per Capita` = `Nominal GDP`)

romania <- read_csv("API_ROU_DS2_en_csv_v2_12273.csv", 
                    skip = 3) |>
...
romania_tabset <- read_csv("API_ROU_DS2_en_csv_v2_12273.csv", 
                           skip = 3) |>
...
romania_school <- read_csv("API_ROU_DS2_en_csv_v2_12273.csv", 
                    skip = 3) |>
...
leaflet(world_measles) |>
  ...
  # center longitude, latitude, zoom level
  setView(lng = 50, lat = 20, zoom = 0.7) |>
...
output$RomaniaData <- renderPlot({
  ...
  scale_color_manual(
    values = c(
      "MCV1 Coverage" = "#ff90b5",
      "Primary School Enrollment" = "#6fa8dc",
      "Primary School Completion Rate" = "#38761d"))
...
poverty level have remained relatively stable or even improved. Interestingly, 
primary school enrollment and completion rates show a similar declining trend to 
MCV1 coverage, suggesting a possible link between education levels, vaccination 
uptake, and the observed increase in measles cases.
:::
